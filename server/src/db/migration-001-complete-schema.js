// Migration script: Create all missing tables, fix seed GUIDs
// Run with: sqlcmd -S "localhost\SQLEXPRESS" -C -E -d NovaPOS -i thisfile.sql
// OR via PowerShell: node server/src/db/migration-001-complete-schema.js
...
-- Delete auto-generated rows and insert with fixed IDs
...
-- 11. Add ReceiptContent column to Sale (for receipt storage)
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Sale') AND name = 'ReceiptContent')
BEGIN
    ALTER TABLE [Sale] ADD [ReceiptContent] nvarchar(max) NULL;
END
...
async function runMigration() {
    ...
    try {
        ...
        console.log('Connected to NovaPOS');
        await pool.request().query(SQL_MIGRATION);
        console.log('Migration 001 applied successfully');

        // Create NovaPosTEST by selecting into database
        const testDbExists = await pool.request()
            .query(`SELECT COUNT(*) AS cnt FROM sys.databases WHERE name = 'NovaPosTEST'`);
        if (testDbExists.recordset[0].cnt === 0) {
            await pool.request().query(`CREATE DATABASE [NovaPosTEST]`);
            console.log('NovaPosTEST database created');
        } else {
            console.log('NovaPosTEST already exists');
        }
    } catch (err) {
